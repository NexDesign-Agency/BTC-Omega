...
export default function App() {
  ...
  // Shared H1 klines for ATR
  const h1KlinesRef = useRef<any[]>([]);
  const m15KlinesRef = useRef<any[]>([]); // FIX: was missing, caused silent crash
  ...
  // Smart alert â€” hanya fire untuk Tier 1/2 confidence >= 75
  ...
  const runAnalysis = async () => {
    ...
    try {
      ...
      // Save H1 & M15 klines to ref so background poller can reuse
      h1KlinesRef.current = h1RawKlines;
      m15KlinesRef.current = m15RawKlines;

      // Compute signal via engine (deterministic, tier-based)
      const sig = computeSignal(realTimeframes, currentPrice, h1RawKlines, m15RawKlines);
      const allSuggestions = computeSignals(realTimeframes, currentPrice, h1RawKlines, m15RawKlines);
      ...
      // BUG 8 FIX: Round zone ke nearest 50 untuk key comparison Ã¢â‚¬â€ cegah voice spam saat harga choppy
  ...
  useEffect(() => {
    ...
    const fetchBackgroundData = async () => {
      try {
        ...
        // FIX: Auto-reset flipFlopMap setiap 10 menit Ã¢â‚¬â€ cegah SIDEWAYS lock permanen
        ...
        if (newPrice && realTimeframes.length === 4) {
          const autoSig = computeSignal(realTimeframes, newPrice, h1KlinesRef.current, m15KlinesRef.current);
          const autoSuggestions = computeSignals(realTimeframes, newPrice, h1KlinesRef.current, m15KlinesRef.current);
          // BUG 8 FIX: Round zone ke nearest 50 Ã¢â‚¬â€ cegah voice spam
          ...
          <div className="flex items-center gap-3 md:gap-4 mb-20 md:mb-24 animate-in fade-in slide-in-from-top-10 duration-700">
            <div className="relative w-10 h-10 md:w-14 md:h-14 bg-bull/10 rounded-xl flex items-center justify-center border border-bull/30">
              <div className="relative text-bull border-2 md:border-4 border-bull rounded-full p-1 flex items-center justify-center w-7 h-7 md:w-10 md:h-10">
                <span className="text-sm md:text-xl font-black italic">ÃŽÂ©</span>
              </div>
            </div>
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-[7px] md:text-[10px] font-bold text-bull/60 tracking-widest uppercase">Live Scanning Ã¢â‚¬Â¢ Binance</span>
              </div>
            </div>
          </div>
    ...
    <div className="h-screen w-screen bg-trading-bg text-slate-300 flex flex-col font-sans overflow-hidden">
        ...
        <div className="flex items-center gap-3 md:gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="hidden sm:inline text-[10px] font-mono text-slate-500 uppercase">BTCUSDT Ã¢â‚¬Â¢ BINANCE</span>
            </div>
          </div>
        </div>
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative">
        ...
        <div className="flex-1 grid grid-cols-12 overflow-hidden relative">
          ...
          <div className={`
            ...
            <div className="flex-1 w-full bg-trading-bg relative overflow-hidden">
              ...
              <AnimatePresence>
                {analysis?.signal && (
                  <>
                    {/* â”€â”€ PRIMARY: BUY / SELL ZONE line â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
                    <motion.div
                    ...
                    {/* â”€â”€ SCALP ZONE line (kuning) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
                    {(() => {
          ...
          <div className={`
            ...
            {(() => {
              const primary = (analysis?.suggestions || []).find(
                s => !s.isSkip && !s.isAdvice && !s.label.includes('SCALP') && !s.label.includes('REVERSAL') && !s.label.includes('RSI') && !s.label.includes('COUNTER') && !s.label.includes('MOMENTUM')
              );
              ...
              return (
                <div className={`m-3 rounded-xl border ${bgColor} ${borderColor} overflow-hidden flex-shrink-0`}>
                  {/* Card Header */}
                  <div className="px-3 py-2 border-b border-white/5 flex justify-between items-center">
                    <span className="text-[10px] uppercase tracking-widest font-black opacity-50">📊 PRIMARY ENTRY</span>
                    {primary && (
                  ...
                  {primary ? (
                    <div className="p-3">
                      ...
                      <div className="grid grid-cols-4 gap-1 mb-2">
                        {[
                          ...
                          { label: 'TP1', value: primary.tp1, col },
                          { label: 'TP2', value: primary.tp2, col },
                        ].map(({ label, value, col }) => (
                    ...
                    <div className="p-3">
                      <p className="text-slate-500 text-[11px]">
                        {analysis?.suggestions?.find(s => s.isSkip)?.reasoning || (analysis ? 'Tidak ada primary setup. Tunggu alignment TF.' : 'Tunggu hasil pemindaian...')}
                      </p>
                    </div>
                  )}
                </div>
            ...
            {/* ── CARD 2: SCALP / REVERSAL ─────────────────────────── */}
            {(() => {
              const scalp = (analysis?.suggestions || []).find(
                s => !s.isSkip && !s.isAdvice && (
                  s.label.includes('SCALP') || s.label.includes('REVERSAL') || s.label.includes('RSI') ||
                  s.label.includes('COUNTER') || s.label.includes('MOMENTUM')
                )
              );
              const isBuy = scalp?.type === 'BUY';
              const isSell = scalp?.type === 'SELL';
              const color = isBuy ? '#00ff88' : isSell ? '#ff4466' : '#eab308';
              const bgColor = 'bg-warning/5';
              const borderColor = scalp ? (isBuy ? 'border-bull/30' : 'border-bear/30') : 'border-warning/20';
              return (
                <div className={`mx-3 mb-3 rounded-xl border ${bgColor} ${borderColor} overflow-hidden flex-shrink-0`}>
                  {/* Card Header */}
                  <div className="px-3 py-2 border-b border-white/5 flex justify-between items-center">
                    <span className="text-[10px] uppercase tracking-widest font-black opacity-50">⚡ SCALP / REVERSAL</span>
                    {scalp && (
                      <div className="flex items-center gap-1.5">
                        <span className="text-[9px] font-mono text-slate-400">CONF</span>
                        <span className="text-sm font-black font-mono" style={{ color }}>{scalp.confidence}%</span>
                        <span className="text-[9px] px-1.5 py-0.5 rounded bg-white/5 border border-white/10 font-mono text-slate-400">RR {scalp.rr}</span>
                      </div>
                    )}
                  </div>
                  ...
                  {scalp ? (
                    <div className="p-3">
                      {/* Label */}
                      <p className="font-black text-base tracking-tight mb-2" style={{ color }}>{scalp.label}</p>

                      {/* Price Levels Grid */}
                      <div className="grid grid-cols-4 gap-1 mb-2">
                        {[
                          { label: 'ENTRY', value: scalp.zone, col: '#94a3b8' },
                          { label: 'SL', value: scalp.sl, col: '#ff4466' },
                          { label: 'TP1', value: scalp.tp1, col },
                          { label: 'TP2', value: scalp.tp2, col },
                        ].map(({ label, value, col }) => (
                          <div key={label} className="p-1.5 bg-trading-bg/80 rounded border border-trading-border text-center">
                            <p className="text-[7px] uppercase tracking-widest mb-0.5" style={{ color: col, opacity: 0.6 }}>{label}</p>
                            <p className="text-[10px] font-mono font-black" style={{ color: col }}>{value}</p>
                      ...
                      {/* Note */}
                      <p className="text-[10px] font-medium leading-snug" style={{ color, opacity: 0.8 }}>{scalp.note}</p>
                    </div>
                  ) : (
                    <div className="p-3">
                      <p className="text-[10px] text-warning/50 font-medium">
                        {analysis ? 'Tidak ada setup scalp/reversal saat ini. Tunggu harga dekat key level.' : 'Tunggu hasil pemindaian...'}
                      </p>
                    </div>
                  )}
                </div>
      ...
      <footer className="h-6 bg-white/[0.02] border-t border-white/[0.05] flex items-center px-4 text-[8px] font-mono tracking-widest text-slate-600 uppercase">
        <div className="flex items-center gap-4 flex-1">
          ...
          <div className="ml-auto">Ã‚Â© 2026 ALPHAPULSE INTEL. LTD. Ã¢â‚¬Â¢ NOT FINANCIAL ADVICE</div>
        </div>
      </footer>
